# -*- coding: utf-8 -*-
"""
Created on Thu Jun 13 22:23:51 2019
...
with open('CV_ER_pax.csv', 'w') as csvfile:
    ...
    filewriter.writerow(['Subject', 'block', 
                     'CV_int01', 'CV_int02', 'CV_int03', 
                     'ER_int01', 'ER_int02', 'ER_int03'])
    for r in rows:
...
plt.scatter([cvs1, cvs2, cvs3], [ers1, ers2, ers3])
plt.xlabel("CV")
plt.ylabel("ER")
plt.show()   
...
import pandas as pd
%matplotlib qt
df1 = pd.DataFrame({'CVint1': cvs1, 'ERint1': ers1})
df2 = pd.DataFrame({'CVint2': cvs2, 'ERint2': ers2})
df3 = pd.DataFrame({'CVint3': cvs3, 'ERint3': ers3})
df4 = pd.DataFrame({'X': cvss, 'y_pred': y_pred})

sns.set_style("whitegrid")
plt.figure()
spl = sns.scatterplot(x="CVint1", y="ERint1", data=df1, s=100, color="steelblue", label="1.45 seconds")
spl = sns.scatterplot(x="CVint2", y="ERint2", data=df2, s=120, marker="s", color="thistle", label="2.9 seconds")
spl = sns.scatterplot(x="CVint3", y="ERint3", data=df3, s=120, marker="d", color="indianred",   label="5.8 seconds")
spl = sns.lineplot(x="X", y="y_pred", data=df4, color="dimgrey", label="Linear regression")

plt.title("Accuracy and Precision (per block, per participant) Linear Regression", fontsize=14)
plt.xlabel("Precision [CV - Coefficient of Variation]", fontsize=14) 
plt.ylabel("Accuracy [ER - Error Rate]", fontsize=14)    

spl.invert_xaxis()
spl.invert_yaxis()
spl.set_ylim(1.20, 0.95)
spl.set_xlim(None, 0.1)
spl.legend(loc='lower right', frameon=True)
#spl.figure(frameon=True)
#plt.grid(b=None)
#spl.figure.savefig("CV_ER_perBlock_perPax.svg")
sns.despine()
# control x and y limits

plt.show()

# How to reverse axis to go from big to small 
...
m=1
for i, j in zip(cvs, ers):
    t = list(zip(i, j))
    xy_chart.add('CV-ER-int0'+str(m), [t[0],  t[1],  t[2],  t[3],  t[4],  t[5], 
...
r, p = stats.pearsonr(cvss, erss) # r = 0.373 p < 0.01
print(r,p)
